...
| B5 | 2026-07-29 | `nix flake update` bumped `set-and-setting` to rev `6d0a50c6` which removed the `lib` output; `nix flake check` fails with `attribute 'lib' missing` because `flake.nix` uses `set-and-setting.lib.*`; additionally `flake.lock` at 120KB exceeded the 65536-byte `.lock` limit | Pin `set-and-setting` back to rev `c6e93cc2` (last version with `lib`); raise `.lock` file size limit to 131072 in `config/lefthook/file_size_limits.yml` |
| B6 | 2026-08-06 | `outputs.nix` was not formatted according to the repository's nixfmt check, causing `guardrails / check` to fail | Format `outputs.nix` with nixfmt |
| B7 | 2026-08-11 | `flake.nix` was not formatted according to the repository's nixfmt check, causing `guardrails / check` to fail | Format `flake.nix` with nixfmt |
| B8 | 2026-08-17 | `lefthook.yml` was generated by the dev-shell hook but ignored and absent from the pull request, causing guardrails fidelity to report a mismatch | Commit the generated `lefthook.yml` artifact |
| B9 | 2026-08-17 | Fragment declarations omitted `actions`, so the committed `lefthook.yml` was generated from a different fragment set than guardrails expected | Include the `actions` fragment in flake and dev-shell assembly, then regenerate `lefthook.yml` |
| B10 | 2026-08-17 | The pinned `set-and-setting` actionlint check evaluates its workflow path regex as a string, but the current check contract requires a list, so `nix flake check` fails before running checks | Keep `actions` in dev-shell materialization for the generated hook, but exclude the incompatible actionlint fragment from the flake check set |
| B11 | 2026-08-17 | The committed hook included `lefthook-actionlint`, but the flake check shell omitted the `actions` fragment; adding that fragment is incompatible with the pinned actionlint check contract, so guardrails coherence reported the command missing from `PATH` | Remove the incompatible actionlint fragment and its generated hook commands from the consumer flake, keeping hook materialization and the supported check set consistent |
| B12 | 2026-08-18 | Guardrails detected the repository workflow and therefore expected the `actions` fragment, but the consumer flake omitted it from every fragment list; generated `lefthook.yml` consequently differed from the guardrails expected assembly | Include `actions` consistently in flake, dev-shell, check, and confirmation materialization inputs, then regenerate the hook |
| B13 | 2026-08-18 | The newly added pinned actionlint check in `set-and-setting` passes a scalar workflow path regex to the current Nix `sourceByRegex` API, so any consumer enabling the `actions` check fragment fails during `nix flake check` with `expected a list but found a string`; the same framework revision also requires its linter-coverage exemption ledger | Exclude `actions` only from the framework-generated flake checks while retaining it in dev-shell materialization, add the required exemption ledger, and raise the Markdown size limit for this bug-history entry |
